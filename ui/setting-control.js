...
export function createSettingControl(opts) {
  ...
  let accounts = opts.accounts ? [...opts.accounts] : [];
  ...
  function renderOverridesEditor() {
    ...
    // add-override row
    const acctSel = h('select', {},
      h('option', { value: '' }, 'All accounts'),
      ...accounts.map((a) => h('option', { value: a.id }, a.label)),
    );
    const yearInp = h('input', { type: 'number', step: '1', placeholder: 'All years', class: 'num yr' });
    ...
    const add = h('button', { onclick: () => {
      ...
      const target = { accountId: acctSel.value || undefined, year: yearInp.value || undefined };
  ...
  function renderPreview() {
    if (!accounts.length) return h('p', { class: 'muted small' }, 'Add accounts to preview per-account resolution.');
  ...
  return {
    el,
    setAccounts(next) { accounts = next ? [...next] : []; render(); },
